...
class Annealer:
    ...
    def grid_search(self):
        ...
        for index, params in param_grid.iterrows():
            ...
            if score > best_score:
                best_score = score
                best_params = dict(zip(self.__search_spaces__.keys(), params))
    ...
    # def simulation_annealing(self, initial_temp = 1., final_temp = 1e-03):
    ...
    def simulation_annealing_fast(self, initial_temp = 1., final_temp = 1e-03):
        print(f"Start SA with {self.__max_iter__} iterations")
        start_time = time.time()
        alpha = self.get_alpha(self.__max_iter__, initial_temp, final_temp)   # Calculate cooling rate alpha
        curr_params = self.get_random_sample()
        best_params = curr_params
        best_score = self.evaluate_params(curr_params)

        temperature = initial_temp
        i = 0
        while temperature > final_temp:
            choice = random.random()
            # With 60% chance select direct neighbor, with 30% flip a parameter to a random value, with 10% select random sample
            next_params = self.get_direct_neighbor(curr_params) if choice < 0.6 else (self.get_param_neighbor(curr_params) if choice < 0.9 else self.get_random_sample())
            next_score = self.evaluate_params(next_params)
        ...
        print(f"Finished {i} SA iterations with best_score={best_score}")
        return best_params, best_score, time.time() - start_time
    
    def get_random_sample(self):
        return {param: random.choice(values) for param, values in self.__search_spaces__.items()}
    
    def get_direct_neighbor(self, current):
        current = copy.deepcopy(current)
        (param, values) = random.choice([(param, values) for param, values in self.__search_spaces__.items() if len(values) > 1])
        current_idx = values.index(current[param])
        new_idx = 1 if current_idx == 0 else (current_idx - 1 if current_idx == len(values) - 1 or random.randint(0, 1) == 0 else current_idx + 1)
        current[param] = values[new_idx]
        return current

    def get_param_neighbor(self, current):
        current = copy.deepcopy(current)
        (param, values) = random.choice([(param, values) for param, values in self.__search_spaces__.items() if len(values) > 1])
        current_idx = values.index(current[param])
        new_idx = random.randint(0, len(values)-2)
